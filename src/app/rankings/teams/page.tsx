...
function TeamMark({ team, large = false }: { team: TeamRow; large?: boolean }) {
  return (
    <div
      className={`flex shrink-0 items-center justify-center overflow-hidden border border-white/15 bg-white/[0.035] ${large ? "h-20 w-20" : "h-12 w-12"}`}
    >
      {team.logo_url ? (
        <Image
          src={team.logo_url}
...
export default async function TeamRankingsPage({
  ...
  const teamsResult = ids.length
    ? await supabase
        .from("teams")
        .select(
          "id,name,short_name,slug,country,logo_url,wins,kills,matches_played,source,verified",
        )
        .in("id", ids)
    : { data: [], error: null };
      ...
      <section className="border-b border-white/15">
        <div className="pr-container py-14 md:py-20">
          ...
          <div className="mt-14 grid gap-10 lg:grid-cols-[1.25fr_.75fr] lg:items-end">
            <div>
              <p className="pr-kicker">Official + verified team layer</p>
              <h1 className="mt-5 text-[clamp(4.3rem,9vw,8.5rem)] font-semibold uppercase leading-[.78] tracking-[-.08em]">
                Team
                <br />
                <span className="text-[var(--pr-red)]">order.</span>
              </h1>
            </div>
            <div className="lg:pb-1">
              <p className="max-w-lg text-base leading-7 text-white/50">
                The current competitive hierarchy of Indian esports teams, with
                form, movement and performance context in one view.
              </p>
      ...
      <section className="border-b border-white/15">
        <div className="pr-container grid grid-cols-2 md:grid-cols-4">
          {[
            ...
            [totalRankings.toLocaleString("en-IN"), "Ranked teams"],
            [(countResult.count || 0).toLocaleString("en-IN"), "Team database"],
        ...
        <section className="pr-container py-16 md:py-24">
          <Reveal>
            <div className="border-b border-white/15 pb-7">
              <p className="pr-kicker">Front runners</p>
        ...
        <div className="pr-container py-16 md:py-20">
          <div className="flex flex-col gap-4 border-b border-white/15 pb-7 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p className="text-xs uppercase tracking-[.15em] text-white/30">
              Score · movement · match record
            </p>
          </div>
          <div>
            {ranked.length ? (
              ranked.map(({ ranking, team }) => (
                <Link
                  key={ranking.id}
                ...
                >
                  ...
                  <RowMetric
                    ...
                    label="Movement"
                  />
                  <RowMetric value={team.wins || 0} label="Wins" />
                  <RowMetric value={team.matches_played || 0} label="Matches" />
                  <RowMetric value={team.kills || 0} label="Kills" />
                  <ArrowRight
                    size={15}
              ...
              <p className="py-14 text-white/40">
                No team rankings are available yet.
              </p>
            )}
          </div>
          <Pagination
      ...
      <section className="pr-container py-14 md:py-18">
        <div className="grid gap-7 border-b border-white/15 pb-12 md:grid-cols-[.7fr_1.3fr]">
          ...
          <p className="max-w-3xl text-sm leading-7 text-white/45">
            Team rankings use official and verified ranking records where
            available, structured alongside roster and match context. They are
            independent intelligence signals—not predictions or official
            tournament decisions. Snapshot shown: {formatDate(snapshot)}.
          </p>
        </div>
      </section>
    </main>
...
function Pagination({
  ...
  return (
    <nav
      className="mt-8 flex items-center justify-between border-t border-white/15 pt-6"
    ...
    <main className="pr-container py-20">
      <section className="border border-[var(--pr-red)]/30 bg-[var(--pr-red)]/5 p-8">
        ...
        <h1 className="mt-4 text-4xl font-semibold">
          Team rankings could not be loaded.
        </h1>
