  ...
  return (
    <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.04] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
      {logoUrl ? 
        ...
        : 
        (
          <span className="text-2xl font-black tracking-tight text-white">{getInitials(name)}</span>
        )
      }
    </div>
  );
...
) 
{
    ...
    return (
      <main className="page-shell py-10">
        <section className={card + " p-8"}>
          ...
          <Link href="/teams" className="mt-5 inline-flex rounded-full border border-white/10 px-5 py-2 text-sm text-white/60 hover:text-white">
            Back to teams
          </Link>
        </section>
      </main>
  ...
  const points = n(currentRanking?.score ?? team.points);
  const wins = n(team.wins);
  const kills = n(team.kills);
  const matchesPlayed = n(team.matches_played);
  const winRate = matchesPlayed > 0 ? Math.round((wins / matchesPlayed) * 100) : 0;
  const killRate = matchesPlayed > 0 ? (kills / matchesPlayed).toFixed(1) : "0.0";
  const powerScore = clamp(Math.round(points * 0.035 + wins * 3.5 + kills * 0.4));
  const firepower = clamp(Math.round(kills * 0.4));
  const winOutput = clamp(Math.round(wins * 8));
  const rankingStrength = currentRanking?.rank? clamp(Math.round(100 - currentRanking.rank * 0.65)) : officialRank? clamp(Math.round(100 - officialRank * 0.65)): 0;
  const consistency = clamp(Math.round(points * 0.04));
  const movementData = calculateMovement(rankHistory);
  const currentRank = currentRanking?.rank || movementData.currentRank || officialRank || 0;
  const archetype = getArchetype({powerScore,firepower,winOutput,consistency,});
  const recentWins = recentMatches.filter((match) => match.winner_team_id === team.id).length;
  ...
  return (
    <main className="page-shell space-y-6 py-8 text-white">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#07080c] p-7 shadow-2xl md:p-9">
        ...
        <div className="relative z-10 flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col gap-5 md:flex-row md:items-center">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#ffd21a]">Team Profile</p>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={`/teams/compare?team1=${team.slug}`}className="rounded-full border border-[#ffd21a]/30 bg-[#ffd21a]/10 px-5 py-2.5 text-sm font-black text-[#ffd21a] transition hover:bg-[#ffd21a]/15">
              Compare Team
            </Link>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        ...
        <Metric label="Rank Change" value={formatChange(currentRanking?.change)} muted />
      ...
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <section className={card + " p-6"}>
          <div className="mb-5 flex items-end justify-between gap-4">
            ...
            <div className="text-right">
              <p className={`text-3xl font-black ${changeTone(movementData.movement)}`}>
                {movementData.movement > 0? `+${movementData.movement}`: movementData.movement}
              </p>
              <p className="text-xs text-white/35">{movementData.direction}</p>
            </div>
          </div>
        ...
        <section className={card + " p-6"}>
          <p className="text-xs font-black uppercase tracking-[0.28em] text-white/35">Team DNA</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <section className={card + " p-6"}>
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-white/35">Active Roster</p>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            {players.length > 0 ? 
              (players.map
                ((player) => 
                  (
                    <Link key={player.id}href={`/players/${player.slug}`}className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 transition hover:border-[#ffd21a]/30 hover:bg-white/[0.05]">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="font-black text-white">{player.ign}</p>
                          <p className="mt-1 text-sm text-white/40">{player.role || "Player"}</p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                        <div>
                          ...
                          <p className="font-bold text-white">{player.avg_damage ?? 0}</p>
                        </div>
                        <div>
                          <p className="text-white/30">Kills</p>
                          <p className="font-bold text-white">{player.total_kills ?? 0}</p>
                        </div>
                        <div>
                          <p className="text-white/30">Form</p>
                          <p className="font-bold text-[#ffd21a]">{player.recent_form || "N/A"}</p>
                        </div>
                      </div>
                    </Link>
                  )
      ...
      <section className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
                ...
                ((match) => 
                  {
                    ...
                    return (
                      <Link
                        key={match.id}
                        href={`/match/${match.id}`}
                        className={`flex h-12 w-12 items-center justify-center rounded-xl border font-black transition hover:-translate-y-0.5 ${
                          didWin
                            ? "border-[#ffd21a]/30 bg-[#ffd21a]/10 text-[#ffd21a]"
                            : "border-white/10 bg-white/[0.04] text-white/50"
                        }`}
                        title={`${match.stage || "Match"} · ${
                          match.map_name || "Map"
                        }`}
                      >
                        {didWin ? "W" : "L"}
        ...
        <section className={card + " p-6"}>
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-white/35">Records</p>
              <h2 className="mt-2 text-2xl font-black text-white">Achievements</h2>
            </div>
            <p className="text-sm text-white/35">{achievements.length} records</p>
          </div>
          <div className="mt-6 space-y-3">
            {achievements.length > 0 ? 
              (achievements.slice(0, 3).map
                ((item) => 
                  (
                    <div key={item.id}className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                      <p className="font-black text-white">{item.title}</p>
                      <p className="mt-1 text-sm text-white/40">{item.tournament_name || "Tournament"}</p>
