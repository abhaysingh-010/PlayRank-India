...
export default async function TeamPage({
  ...
  const [
    ...
    supabase
      .from("players")
      .select("id,ign,slug,role,kd_ratio,avg_damage,total_kills,recent_form")
      .eq("team_id", team.id)
      .order("ign", { ascending: true }),
    supabase
      .from("matches")
      .select(
        "id,winner_team_id,team1_score,team2_score,map_name,stage,date,team1:team1_id(name),team2:team2_id(name)",
      )
  ...
  const wins = num(team.wins),
    ...
    played = num(team.matches_played);
  ...
  const recentWins = matches.filter(
    ...
    <main className="bg-[var(--pr-bg)] text-white">
      <section className="border-b border-white/15">
        <div className="pr-container py-12 md:py-18">
          ...
          >
            <ArrowLeft size={13} /> Team directory
          </Link>
            ...
            <div className="flex flex-col gap-7 sm:flex-row sm:items-end">
              ...
              <div>
                <p className="pr-kicker">
                  Team dossier · {team.country || "India"}
                </p>
                <h1 className="mt-4 max-w-5xl text-[clamp(3.7rem,8vw,8rem)] font-semibold uppercase leading-[.8] tracking-[-.075em]">
                  {team.name}
                </h1>
                <p className="mt-5 text-xs font-bold uppercase tracking-[.17em] text-white/35">
                  {team.short_name || "Team"} · {sourceLabel(team.source)} ·{" "}
      ...
      <section className="border-b border-white/15">
        <div className="pr-container grid grid-cols-2 md:grid-cols-6">
          {[
            ...
            [change(ranking?.change), "Movement"],
            [wins, "Wins"],
      ...
      <section className="pr-container grid gap-12 py-18 lg:grid-cols-[1.15fr_.85fr]">
        ...
        <Reveal delay={0.08}>
          <div>
            <p className="pr-kicker">Competitive read</p>
            <h2 className="mt-5 text-4xl font-semibold tracking-[-.055em]">
              {rank && rank <= 10
                ? "A front-running roster."
                : winRate >= 25
                  ? "A team building momentum."
                  : "A developing competitive record."}
            </h2>
            <p className="mt-5 text-sm leading-7 text-white/45">
              {team.name} has recorded {wins} wins and {kills} kills across{" "}
              {played} tracked matches. The current record produces a {winRate}%
              win rate with {killsPerMatch} kills per match.
            </p>
            <div className="mt-8 grid grid-cols-2 border border-white/15">
              <Insight
                value={`${recentWins}/${matches.length}`}
                label="Recent wins"
              />
              <Insight value={players.length} label="Roster size" />
              <Insight value={achievements.length} label="Achievements" />
              <Insight
                value={team.verified ? "Verified" : "Recorded"}
                label="Data status"
              />
            </div>
          </div>
      ...
      <section className="border-y border-white/15 bg-[var(--pr-surface)]">
        <div className="pr-container py-16">
          <div className="flex items-end justify-between border-b border-white/15 pb-7">
            <div>
              <p className="pr-kicker">Active roster</p>
              <h2 className="mt-4 text-4xl font-semibold tracking-[-.05em]">
                The lineup.
              </h2>
            </div>
            <p className="text-xs text-white/30">{players.length} players</p>
          </div>
            ...
            {players.length ? (
              players.map((player) => (
                <Link
                  ...
                  className="group grid grid-cols-[48px_1fr_auto] items-center gap-4 border-b border-white/10 py-5 md:odd:border-r md:odd:pr-7 md:even:pl-7"
                >
                  <div className="flex h-12 w-12 items-center justify-center border border-white/15 text-xs font-black text-white/55">
                    {initials(player.ign)}
                  </div>
                  <div>
                    <p className="font-semibold">{player.ign}</p>
                    <p className="mt-1 text-[9px] uppercase tracking-[.15em] text-white/25">
                      {player.role || "Player"}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold">
                      {player.total_kills || 0} kills
                    </p>
              ...
              <p className="py-12 text-white/35">
                No roster data is available yet.
              </p>
            )}
          </div>
        </div>
      ...
      <section className="pr-container grid gap-12 py-18 lg:grid-cols-2">
        <div>
          <div className="flex items-center justify-between border-b border-white/15 pb-6">
            <div>
              <p className="pr-kicker">Recent form</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-.04em]">
                Last five.
              </h2>
            </div>
            <Link href="/matches" className="text-white/30 hover:text-white">
          ...
          {matches.length ? (
            matches.map((match) => (
              <Link
                key={match.id}
                href={`/match/${match.id}`}
                className="group grid grid-cols-[42px_1fr_auto] items-center gap-4 border-b border-white/10 py-5"
              >
                <span
                  className={`flex h-9 w-9 items-center justify-center border text-xs font-black ${match.winner_team_id === team.id ? "border-[var(--pr-gold)] text-[var(--pr-gold)]" : "border-white/15 text-white/35"}`}
                >
                  {match.winner_team_id === team.id ? "W" : "L"}
                </span>
                <div>
                  <p className="font-semibold">
                    {match.team1?.[0]?.name || "Team"} vs{" "}
                    {match.team2?.[0]?.name || "Team"}
                  </p>
                  <p className="mt-1 text-[9px] uppercase tracking-[.14em] text-white/25">
                    {match.stage || "Match"} · {match.map_name || "Map"} ·{" "}
                    {formatDate(match.date)}
                  </p>
                </div>
                <ArrowRight
                  size={14}
                  className="text-white/20 group-hover:text-[var(--pr-red)]"
                />
              </Link>
            ))
          ) : (
            <p className="py-10 text-white/35">No recent matches found.</p>
          )}
        </div>
        <div>
          <div className="flex items-center justify-between border-b border-white/15 pb-6">
            <div>
              <p className="pr-kicker">Honours</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-[-.04em]">
                Achievements.
              </h2>
            </div>
          ...
          {achievements.length ? (
            achievements.slice(0, 5).map((item) => (
              <div
                key={item.id}
                className="grid grid-cols-[1fr_auto] gap-5 border-b border-white/10 py-5"
              >
                <div>
                ...
                <div className="text-right">
                  <p className="font-semibold text-[var(--pr-gold)]">
            ...
            <p className="py-10 text-white/35">No achievements recorded yet.</p>
          )}
        </div>
      </section>

      <section className="pr-container pb-18">
        <div className="grid gap-6 border-y border-white/15 py-8 md:grid-cols-[auto_1fr] md:items-start">
          <ShieldCheck size={20} className="text-[var(--pr-red)]" />
          <p className="max-w-4xl text-sm leading-7 text-white/40">
            This dossier combines the latest ranking snapshot with available
            team, roster, achievement and match records. Source:{" "}
            {sourceLabel(team.source)}. PlayRank analytics are independent
            competitive intelligence and not outcome predictions.
          </p>
        </div>
      </section>
    </main>
...
function TeamMark({ team }: { team: Team }) {
  return (
    <div className="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden border border-white/15 bg-white/[0.035]">
      {team.logo_url ? (
        <Image
          src={team.logo_url}
          alt={`${team.name} logo`}
