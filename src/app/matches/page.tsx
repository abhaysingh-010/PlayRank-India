      ...
      <div className="min-w-0">
        {team?.slug ? (
          <Link
            ...
            className="truncate font-black text-white hover:underline"
          >
            {teamName}
          </Link>
        ) : (
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4">
        <p className="text-sm text-white/40">
          Team result rows are not available for this match yet.
        </p>
      </div>
    );
  }
    ...
    <article className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#090b10] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.28)] transition hover:-translate-y-0.5 hover:border-[#ffd21a]/30 hover:bg-white/[0.035]">
      ...
      <div className="relative z-10">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap gap-2">
              <DataSourceBadge
                ...
                label={
                  match.source === "pubg_developer_api"
                    ? "PUBG API Promoted"
                    : match.verified
                      ? "Verified Match"
                      : "Match Record"
            ...
            <p className="mt-3 text-xs font-black uppercase tracking-[0.24em] text-white/35">
              {match.stage || "Match"}
            </p>
          ...
          <p className="shrink-0 rounded-full border border-white/10 bg-white/[0.035] px-3 py-1 text-xs text-white/45">
        ...
        <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
          <div>
            ...
            <p className="mt-1 font-black text-[#ffd21a]">
              {match.winner?.name ||
                resultRows.find((row) => row.placement === 1)?.team?.name ||
                "Pending"}
            </p>
          </div>
...
export default async function MatchesPage() {
  ...
  for (const row of matchResults) {
    const existing = resultsByMatchId.get(row.match_id) || [];
    existing.push(row);
    resultsByMatchId.set(row.match_id, existing);
  }
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {matches.map((match) => (
              <MatchCard
                key={match.id}
                match={match}
                resultRows={resultsByMatchId.get(match.id) || []}
              />
            ))}
          </div>
        ) : (
