...
export async function POST(request: NextRequest) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json(
      { ok: false, error: "CSV or JSON file is required" },
      { status: 400 },
    );
  }
  if (!tournamentName) {
    return NextResponse.json(
      { ok: false, error: "Tournament name is required" },
      { status: 400 },
    );
  }
  ...
  const [
    ...
    supabaseAdmin
      ...
      .select("team_id,alias_slug")
      .eq("verified", true),
    supabaseAdmin
      .from("player_aliases")
      .select("player_id,alias_slug")
      .eq("verified", true),
  ]);

  const teamMap = new Map<string, string>();
  for (const team of (teams ?? []) as Team[]) {
    [team.name, team.short_name, team.slug]
      .filter(Boolean)
      .forEach((key) => teamMap.set(normalize(key), team.id));
  }
  for (const alias of (teamAliases ?? []) as Alias[])
    teamMap.set(normalize(alias.alias_slug), alias.team_id);

  const playerMap = new Map<string, string>();
  for (const player of (players ?? []) as Player[]) {
    [player.ign, player.slug].forEach((key) =>
      playerMap.set(normalize(key), player.id),
    );
  }
  for (const alias of (playerAliases ?? []) as PlayerAlias[]) {
    if (alias.player_id)
      playerMap.set(normalize(alias.alias_slug), alias.player_id);
  }
  ...
  const prepared = rows.map((row, index) => {
    ...
    const playerId = playerIgn
      ? (playerMap.get(normalize(playerIgn)) ?? null)
      : null;
    const errors: string[] = [];
    ...
    if (recordType === "player_stat" && !playerId)
      errors.push(`Unmapped player: ${playerIgn}`);
  ...
  const batchStatus =
    counts.invalid || counts.unresolved ? "needs_review" : "validated";
  ...
  if (batchError) {
    return NextResponse.json(
      { ok: false, error: batchError.message },
      { status: 500 },
    );
  }
  ...
  const { error: rawError } = await supabaseAdmin
    ...
    .insert({
      ...
      source_url: sourceUrl || null,
      source_type: file.name.toLowerCase().endsWith(".json")
        ? "json_batch"
        : "csv_batch",
  ...
  if (rawError || rowsError) {
    await supabaseAdmin
      ...
      .update({
        status: "failed",
        failed_records: prepared.length,
    ...
    return NextResponse.json(
      {
        ok: false,
        batch_id: batchId,
        error: rawError?.message || rowsError?.message,
      },
      { status: 500 },
    );
  }
