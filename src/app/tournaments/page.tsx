...
type CountRow = { tournament_id: string | null };
...
function pageHref(page: number, sort: SortDirection) {
  return `/tournaments?page=${page}&sort=${sort}`;
}
...
export default async function TournamentsPage({
  ...
  const [eventsResult, matchesResult, standingsResult] = await Promise.all([
    supabase
      .from("tournaments")
      .select(
        "id,name,slug,organizer,status,location,prize_pool,participating_teams,start_date,end_date,verified",
        { count: "exact" },
      )
  ...
  const count = (rows: CountRow[], id: string) =>
    rows.filter((row) => row.tournament_id === id).length;
        ...
        <div className="pr-container grid gap-12 py-16 md:py-24 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
          ...
          <div>
            <p className="max-w-xl text-base leading-7 text-white/50">
              Track Indian esports events through dates, status, prize pools,
              match coverage and published standings.
            </p>
      ...
      <section className="pr-container py-16 md:py-22">
        <div className="flex flex-col gap-6 border-b border-white/15 pb-7 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex border border-white/15 p-1" aria-label="Tournament date order">
              <Link
                href={pageHref(1, "desc")}
                aria-current={sort === "desc" ? "page" : undefined}
                className={`px-4 py-3 text-[9px] font-semibold uppercase tracking-[.14em] transition-colors ${
                  sort === "desc"
                    ? "bg-[var(--pr-red)] text-black"
            ...
            events.map((event) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-1 text-[9px] uppercase tracking-[.14em] text-white/25">
                    {event.organizer || "Organiser TBD"} · {event.location || "India"}
                  </p>
                </div>
                <Stat value={event.status || "Upcoming"} label="Status" />
                <Stat value={money(event.prize_pool)} label="Prize pool" />
                <Stat
                  value={`${count(matches, event.id)} / ${count(standings, event.id)}`}
        ...
        <nav className="mt-8 flex items-center justify-between border-t border-white/15 pt-7">
          <p className="text-[10px] uppercase tracking-[.16em] text-white/30">
            Page {Math.min(page, pages)} of {pages} · {sort === "asc" ? "Oldest first" : "Newest first"}
          </p>
