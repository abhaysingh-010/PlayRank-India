...
export default async function MissingSlugsPage() {
  ...
  const error =
    teamsResult.error || playersResult.error || tournamentsResult.error;
  ...
  const issues: MissingSlugIssue[] = [
    ...
    ...tournaments.map((tournament) => ({
      ...
      adminHref: `/admin/tournaments?search=${encodeURIComponent(
        tournament.name
      )}`,
    })),
  ];
      ...
      <main className="page-shell py-10 text-white">
        <section className="rounded-[2rem] border border-red-500/20 bg-red-500/5 p-8">
          ...
          <p className="mt-3 text-red-200/80">
            Failed to load missing-slug records.
          </p>
    ...
    <main className="page-shell space-y-6 py-8 text-white">
      <section className="rounded-[2rem] border border-white/10 bg-[#07080c] p-7 md:p-9">
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          ...
          <Link href="/admin/teams" className="btn-primary px-5 py-3 text-sm">
            Admin Teams
          </Link>
          <Link href="/admin/players" className="btn-secondary px-5 py-3 text-sm">
            Admin Players
          </Link>
          <Link
                ...
                issues.map((issue) => (
                  ...
                  >
                    <td className="px-6 py-5">
                      <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-white/60">
                        {issue.type}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <code className="rounded-lg border border-white/10 bg-black/30 px-3 py-1.5 text-sm text-[#ffd21a]">
                        {issue.suggestedSlug || "manual-slug-required"}
                      </code>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <DataSourceBadge
                        ...
                        label={getSourceLabel(issue.source, issue.verified)}
                      />
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <p className="text-lg font-black text-emerald-300">
                      No missing slugs found.
                    </p>
                    <p className="mt-2 text-sm text-white/45">
                      Team, player and tournament routes are currently clean.
                    </p>
                  </td>
                </tr>
              )}
