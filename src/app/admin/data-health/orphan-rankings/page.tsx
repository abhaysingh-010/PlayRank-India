...
type OrphanRankingIssue = RankingRow & {reason: string;};
...
export default async function OrphanRankingsPage() 
{
  ...
  (
    [
      teamIds.length > 0 ? supabaseAdmin.from("teams").select("id").in("id", teamIds) : Promise.resolve({ data: [], error: null }),

      playerIds.length > 0 ? supabaseAdmin.from("players").select("id").in("id", playerIds) : Promise.resolve({ data: [], error: null }),
    ]
  );

  const error = rankingsError || teamsResult.error || playersResult.error;

  const existingTeamIds = new Set(((teamsResult.data || []) as EntityRow[]).map((row) => row.id));

  const existingPlayerIds = new Set(((playersResult.data || []) as EntityRow[]).map((row) => row.id));
    ...
    ((ranking) => 
      {
        ...
        if 
        (
          ranking.entity_type === "player" && !existingPlayerIds.has(ranking.entity_id)
        ) 
        {
          return {
            ...ranking,
    ...
    return (
      <main className="page-shell py-10 text-white">
        <section className="rounded-[2rem] border border-red-500/20 bg-red-500/5 p-8">
          ...
          <p className="mt-3 text-red-200/80">Failed to load orphan-ranking records.</p>
    ...
    <main className="page-shell space-y-6 py-8 text-white">
      <section className="rounded-[2rem] border border-white/10 bg-[#07080c] p-7 md:p-9">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              ...
              <p className="mt-2 text-3xl font-black text-[#ffd21a]">{orphanRankings.length.toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          ...
          <Link href="/admin/rankings-sync"className="btn-primary px-5 py-3 text-sm">Open Rankings Sync</Link>
        </div>
      </section>
                  ...
                  (
                    ...
                    (
                      <tr key={ranking.id}className="border-b border-white/[0.06] transition hover:bg-white/[0.025]">
                        ...
                        <td className="px-6 py-5">
                          <code className="rounded-lg border border-white/10 bg-black/30 px-3 py-1.5 text-xs text-white/60">{ranking.entity_id}</code>
                        </td>
                        <td className="px-6 py-5 text-right font-black">{ranking.rank ? `#${ranking.rank}` : "—"}</td>
                        <td className="px-6 py-5 text-right text-white/70">{ranking.score ?? "—"}</td>
                        <td className="px-6 py-5 text-right text-white/55">{formatChange(ranking.change)}</td>
                        <td className="px-6 py-5 text-white/45">{formatDate(ranking.updated_at)}</td>
                        <td className="px-6 py-5 text-sm text-white/50">{ranking.reason}</td>
                        <td className="px-6 py-5 text-right">
                          <Link href="/admin/rankings-sync" className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white/60 transition hover:border-[#ffd21a]/30 hover:text-[#ffd21a]">
                            Review
                          </Link>
                        </td>
                      </tr>
                ...
                (
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center">
                      <p className="text-lg font-black text-emerald-300">No orphan rankings found.</p>
                      <p className="mt-2 text-sm text-white/45">Ranking entity references are currently clean.</p>
                    </td>
                  </tr>
                )
              }
