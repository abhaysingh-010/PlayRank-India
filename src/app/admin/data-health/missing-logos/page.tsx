    ...
    return (
      <main className="page-shell py-10 text-white">
        <section className="rounded-[2rem] border border-red-500/20 bg-red-500/5 p-8">
          ...
          <p className="mt-3 text-red-200/80">Failed to load missing-logo records.</p>
    ...
    <main className="page-shell space-y-6 py-8 text-white">
      <section className="rounded-[2rem] border border-white/10 bg-[#07080c] p-7 md:p-9">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              ...
              <p className="mt-2 text-3xl font-black text-[#ffd21a]">{teams.length.toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          ...
          <Link href="/admin/teams" className="btn-primary px-5 py-3 text-sm">Open Admin Teams</Link>
        </div>
      </section>
              ...
              {teams.length > 0 ?
                (
                  ...
                  (
                    (team) =>
                    (
                      <tr key={team.id}className="border-b border-white/[0.06] transition hover:bg-white/[0.025]">
                        ...
                        <td className="px-6 py-5 font-mono text-sm text-white/55">{team.slug}</td>
                        <td className="px-6 py-5 text-white/55">{team.country || "India"}</td>
                        <td className="px-6 py-5">
                          <DataSourceBadge source={team.source}verified={team.verified}label={getTeamSourceLabel(team)}/>
                        </td>
                        <td className="px-6 py-5">
                          {team.active === false ?
                            (
                        ...
                        <td className="px-6 py-5 text-white/45">{formatDate(team.created_at)}</td>
                        <td className="px-6 py-5 text-right">
                          <Link href={`/admin/teams?search=${encodeURIComponent(team.name)}`}className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white/60 transition hover:border-[#ffd21a]/30 hover:text-[#ffd21a]">
                            Fix
                          </Link>
                        </td>
                      </tr>
                ...
                (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center">
                      <p className="text-lg font-black text-emerald-300">No missing team logos found.</p>
                      <p className="mt-2 text-sm text-white/45">This data health check is currently clean.</p>
                    </td>
                  </tr>
                )
              }
