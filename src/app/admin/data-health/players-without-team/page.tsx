...
export default async function PlayersWithoutTeamPage() 
{
  ...
  .from("players")
  .select("id, ign, real_name, slug, team_id, role, country, matches_played, total_kills, source, verified, active, created_at")
  .is("team_id", null)
  ...
  const highPriorityCount = players.filter((player) => player.active === true && n(player.matches_played) >= 5).length;
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              ...
              <p className="mt-2 text-3xl font-black text-[#ffd21a]">{players.length.toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/35">High Priority</p>
              <p className="mt-2 text-3xl font-black text-red-300">{highPriorityCount.toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/35">Active Players</p>
              <p className="mt-2 text-3xl font-black">{activeCount.toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-white/35">Verified</p>
              <p className="mt-2 text-3xl font-black">{verifiedCount.toLocaleString("en-IN")}</p>
            </div>
          </div>
        </div>
                ...
                (
                  ...
                  (
                    (player) => 
                    {
                      ...
                      return (
                        <tr key={player.id} className="border-b border-white/[0.06] transition hover:bg-white/[0.025]">
                          <td className="px-6 py-5">
                            <p className="font-black text-white">{player.ign}</p>
                            <p className="mt-1 text-xs uppercase tracking-[0.18em] text-white/35">{player.real_name || player.country || "Player"}</p>
                          </td>
                          <td className="px-6 py-5 font-mono text-sm text-white/55">{player.slug || "missing-slug"}</td>
                          <td className="px-6 py-5 text-white/55">{player.role || "—"}</td>
                          <td className="px-6 py-5 text-right text-white/65">{n(player.matches_played).toLocaleString("en-IN")}</td>
                          <td className="px-6 py-5 text-right text-white/65">{n(player.total_kills).toLocaleString("en-IN")}</td>
                          <td className="px-6 py-5">
                            <DataSourceBadge source={player.source} verified={player.verified} label={getPlayerSourceLabel(player)}/>
                          </td>
                          <td className="px-6 py-5">
                            <span className={`rounded-full border px-3 py-1 text-xs font-bold ${priority.className}`}title={priority.reason}>
                              {priority.label}
                            </span>
                          </td>
                          <td className="px-6 py-5 text-white/45">{formatDate(player.created_at)}</td>
                          <td className="px-6 py-5 text-right">
                            <Link href={`/admin/players?search=${encodeURIComponent(player.ign)}`}className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white/60 transition hover:border-[#ffd21a]/30 hover:text-[#ffd21a]">
                              Fix
                            </Link>
                          </td>
                        </tr>
                ...
                (
                  <tr>
                    <td colSpan={9} className="px-6 py-12 text-center">
                      <p className="text-lg font-black text-emerald-300">No players without team links found.</p>
                      <p className="mt-2 text-sm text-white/45">Player roster links are currently clean.</p>
                    </td>
                  </tr>
                )
              }
