...
[
  {
    ...
    description:"Check table counts, missing logos, broken slugs, orphan rankings, import health and promotion readiness.",
  ...
  },
  {
    ...
    description:"Audit player-team sync, active roster records and PUBG promotion safety.",
  ...
  },
  {
    ...
    description:"Import PUBG API match data into staging without touching public PlayRank core tables.",
  ...
  },
  {
    title: "PUBG Imports",
    label: "Import Review",
    description:"Review imported API matches, blocked imports, mapped players and promotion eligibility.",
  ...
  },
  {
    title: "Matches",
    label: "Match Ops",
    description:"Manage structured match records powering PlayRank match intelligence.",
    href: "/admin/matches",
    status: "Protected",
    tone: "neutral",
    group: "Content Ops",
  ...
  },
  {
    title: "Data Trust",
    label: "Source Layer",
    description:"Review public-facing source context, imports, official data notes and ranking trust layer.",
    href: "/data",
    status: "Live",
    tone: "healthy",
    group: "Public Review",
...
) 
{
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
      ...
      <p className={`mt-2 text-3xl font-black ${tone === "healthy"? "text-emerald-300": tone === "warning"? "text-yellow-300" : tone === "danger"? "text-red-300" : "text-white"}`}>
        {value}
      </p>
    </div>
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#ffd21a]">{item.label}</p>
            <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${toneClass(item.tone)}`}>{item.status}</span>
          </div>
          <h3 className="mt-3 text-xl font-black text-white">{item.title}</h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">{item.description}</p>
        </div>
        <span className="shrink-0 text-sm font-black text-white/25 transition group-hover:text-[#ffd21a]">Open</span>
      </div>
    </Link>
  );
}
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-[#080a0f] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
      <div className="mb-5 flex items-center justify-between border-b border-white/10 pb-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.25em] text-[#ffd21a]">Module Group</p>
          <h2 className="mt-2 text-2xl font-black text-white">{title}</h2>
        </div>
        <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-black text-white/45">{items.length}</span>
      </div>
...
{
  const protectedModules = adminModules.filter((module) => module.status === "Protected").length;
  const liveModules = adminModules.filter((module) => module.status === "Live").length;
  const warningModules = adminModules.filter((module) => module.tone === "warning").length;
  return (
    <main className="min-h-screen bg-[#030406] text-white">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/admin/data-health" className="rounded-full border border-[#ffd21a]/30 bg-[#ffd21a]/10 px-5 py-2.5 text-sm font-black text-[#ffd21a] transition hover:bg-[#ffd21a]/15">
                  Data Health
                </Link>
                <Link href="/admin/pubg" className="rounded-full border border-white/10 bg-white/[0.04] px-5 py-2.5 text-sm font-black text-white/65 transition hover:border-white/25 hover:text-white">
                  PUBG Hub
                </Link>
                <Link href="/admin/pubg/mappings" className="rounded-full border border-white/10 bg-white/[0.04] px-5 py-2.5 text-sm font-black text-white/65 transition hover:border-white/25 hover:text-white">
                  Mappings
                </Link>
              </div>
            </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-5 px-5 py-10 lg:grid-cols-2">
        {groups.map
          ((group) => 
            (
              <GroupSection key={group}title={group}items={adminModules.filter((module) => module.group === group)}/>
            )
          )
        }
      </section>
      <section className="border-t border-white/10 bg-[#050609]">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 md:flex-row md:items-center md:justify-between">
          ...
          <Link href="/data" className="text-sm font-black text-white/40 transition hover:text-[#ffd21a]">Review Data Trust</Link>
        </div>
      </section>
    </main>
  );
