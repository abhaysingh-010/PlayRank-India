      ...
      <section className="krafton-grid relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#07080c] p-8 md:p-12">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Link href="/admin/players" className="btn-primary px-6 py-3 text-sm">
              Admin Players
            </Link>

            <Link href="/admin/teams" className="btn-secondary px-6 py-3 text-sm">
              Admin Teams
            </Link>
            ...
            <Link href="/admin/rosters/health"className="btn-secondary px-6 py-3 text-sm">
              Roster Health
            </Link>
          </div>
        </div>
      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <form action={addRoster} className="krafton-card p-7">
          <div className="space-y-4">
            ...
            <input
              name="role"
              placeholder="Role, e.g. IGL / Assaulter / Support"
              className="w-full rounded-xl border border-white/10 bg-black px-4 py-4 text-white outline-none placeholder:text-white/35"
            />

            <button
              type="submit"
              className="w-full rounded-xl border border-emerald-400/25 bg-emerald-400/10 px-5 py-4 text-sm font-black uppercase tracking-[0.16em] text-emerald-300 hover:bg-emerald-400/20"
            >
              Link Player To Team
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          {activeRosters.map((roster) => {
            ...
            return (
              <div
                key={roster.id}
                className="grid gap-4 border border-white/10 bg-white/[0.03] p-5 md:grid-cols-[1fr_1fr_1fr_auto]"
              >
                <div>
                ...
                <form action={deactivateRoster}>
                  ...
                  <button
                    type="submit"
                    className="rounded-xl border border-red-400/25 bg-red-400/10 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-red-300 hover:bg-red-400/20"
                  >
                    Deactivate
                  </button>
                </form>
