        ...
        <div className="mx-auto grid max-w-[1500px] gap-8 px-5 py-10 md:px-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#f4473b]">
              Relationship management / rosters
            </p>
        ...
        <section className="mt-6 grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
          <div className="border border-white/10 bg-[#080a0f] p-6">
            ...
            <p className="mt-4 text-sm leading-6 text-white/45">
              ...
              player&apos;s direct `team_id`. Existing active roster links for
              that player are closed automatically.
            </p>
          </div>
          <form
            action={addRoster}
            className="border border-white/10 bg-[#080a0f] p-6"
          >
            <div className="grid gap-5 md:grid-cols-2">
              <label className="space-y-2">
                ...
                <select
                  name="player_id"
                  required
                  className="w-full border border-white/10 bg-[#050609] px-4 py-3.5 text-sm text-white outline-none focus:border-[#ffd21a]/60"
                >
              ...
              <label className="space-y-2 md:col-span-2">
                ...
                <input
                  ...
                  className="w-full border border-white/10 bg-[#050609] px-4 py-3.5 text-sm text-white outline-none placeholder:text-white/20 focus:border-[#ffd21a]/60"
                />
              </label>
              <button
                type="submit"
                className="bg-[#f4473b] px-5 py-4 text-xs font-black uppercase tracking-[0.18em] text-black transition hover:bg-[#ff5a4f] md:col-span-2"
              >
                Link Player To Team
              </button>
            </div>
            ...
            {activeRosters.length === 0 ? (
              <div className="border border-yellow-400/20 bg-yellow-400/10 p-6">
                <p className="font-black uppercase text-yellow-300">
                  No active roster links found.
                </p>
              </div>
            ) : null}
            {activeRosters.map((roster) => {
              const player = roster.player_id
                ? playerById.get(roster.player_id)
                : null;
              const team = roster.team_id ? teamById.get(roster.team_id) : null;
              return (
                <div
                  key={roster.id}
                  className="grid gap-4 border border-white/10 bg-[#080a0f] p-5 md:grid-cols-[1fr_1fr_1fr_auto] md:items-center"
                >
                  <div>
                  ...
                  <div>
                    ...
                    <p className="mt-2 font-black text-white">
                      {roster.role || player?.role || "Unassigned"}
                    </p>
                  </div>
                  <form action={deactivateRoster}>
                    ...
                    <button
                      type="submit"
                      className="border border-red-400/25 bg-red-400/10 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-red-300 hover:bg-red-400/20"
                    >
                      Deactivate
                    </button>
                  </form>
