      ...
      <header className="border-b border-white/10 bg-[#050609]">
        <div className="mx-auto grid max-w-[1500px] gap-8 px-5 py-10 md:px-8 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/admin/data-health/missing-logos" className="pr-button pr-button-secondary">
              Logo issues
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-[1500px] gap-6 px-5 py-10 md:px-8 xl:grid-cols-[0.72fr_1.28fr]">
        <aside className="border border-white/10 bg-[#080a0f] p-6">
          ...
          <h2 className="mt-4 text-3xl font-black uppercase tracking-[-0.05em]">
            Create once.<br />Connect everywhere.
          </h2>
          <div className="mt-8 space-y-5 border-t border-white/10 pt-6 text-sm leading-6 text-white/45">
            <p><span className="font-bold text-white">01.</span> Name generates the public slug automatically.</p>
            <p><span className="font-bold text-white">02.</span> New records start active and unverified.</p>
            <p><span className="font-bold text-white">03.</span> Roster and source mappings can be attached afterward.</p>
          </div>
        </aside>
        ...
        <section className="border border-white/10 bg-[#080a0f]">
          ...
          <div className="grid gap-5 p-5 md:grid-cols-2 md:p-6">
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-white/45">Team name *</span>
              <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Team Soul" className={inputClass} />
            </label>
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-white/45">Short name</span>
              <input value={shortName} onChange={(event) => setShortName(event.target.value)} placeholder="SOUL" className={inputClass} />
            </label>
            <label className="space-y-2">
            ...
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-white/45">Logo URL</span>
              <input value={logoUrl} onChange={(event) => setLogoUrl(event.target.value)} placeholder="https://…" className={inputClass} />
            </label>
          </div>
          ...
          <div className="grid gap-4 border-t border-white/10 p-5 md:grid-cols-[1fr_auto] md:items-center md:p-6">
            <div>
              ...
              <p className="mt-2 break-all font-mono text-sm text-white/60">/teams/{slug || "team-slug"}</p>
            </div>
            <button onClick={addTeam} disabled={loading} className="bg-[#f4473b] px-7 py-4 text-xs font-black uppercase tracking-[0.18em] text-black transition hover:bg-[#ff5a4f] disabled:cursor-not-allowed disabled:opacity-45">
