...
export default function BgmiBatchImportForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResult({
        ok: false,
        error: "Upload failed. Check the server connection and retry.",
      });
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={submit} className="space-y-5">
      <label className="block">
        ...
        <input
          ...
          className="mt-2 w-full border border-white/10 bg-black/35 px-4 py-3 text-white outline-none transition focus:border-[#f4473b]"
        />
      </label>
      ...
      <label className="group flex min-h-44 cursor-pointer flex-col items-center justify-center border border-dashed border-white/20 bg-white/[0.025] px-6 text-center transition hover:border-[#f4473b]/70 hover:bg-[#f4473b]/[0.04]">
        ...
        <input
          ...
          required
          className="mt-5 block max-w-full text-xs text-white/45 file:mr-3 file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-white"
        />
      </label>

      <button
        disabled={loading}
        className="flex w-full items-center justify-center gap-2 bg-[#f4473b] px-5 py-4 text-xs font-black uppercase tracking-[0.18em] text-black transition hover:bg-[#ff5a4e] disabled:opacity-50"
      >
        {loading ? (
      ...
      {result && (
        <div
          className={`border p-5 ${result.ok ? "border-emerald-400/25 bg-emerald-400/[0.06]" : "border-red-400/25 bg-red-400/[0.06]"}`}
        >
          <div className="flex items-center gap-2">
            ...
            <p className="font-black text-white">
              {result.ok ? "Batch staged successfully" : "Import failed"}
            </p>
          </div>
          {result.ok ? (
            <>
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                  ...
                  ["Unresolved", result.unresolved_rows ?? 0],
              ...
              {result.batch_id && (
                <Link
                  href={"/admin/bgmi/imports/" + result.batch_id}
                  className="mt-4 inline-flex bg-white px-4 py-3 text-xs font-black uppercase tracking-widest text-black"
                >
