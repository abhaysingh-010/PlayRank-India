    ...
    <header className="pr-nav">
      <div className="pr-container flex h-[72px] items-center justify-between gap-8">
          ...
          {primaryLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`pr-nav-link ${active ? "is-active" : ""}`}
              >
                {link.label}
              </Link>
            );
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Link
            href="/compare"
            className="pr-button pr-button-primary h-10 px-5 text-[11px]"
          >
            Compare
          </Link>
        </div>
      ...
      {menuOpen ? (
        <div
          ...
          className="pr-mobile-menu absolute inset-x-0 top-full h-[calc(100dvh-72px)] overflow-y-auto border-t border-white/10 bg-[var(--pr-bg)] lg:hidden"
        >
          <nav
            className="pr-container grid gap-0 py-4 pb-10"
