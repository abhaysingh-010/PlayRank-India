...
begin
  ...
  ranked_teams as (
    select
      id,
      row_number() over (order by score desc, recent_matches desc, name asc)::int as rank,
      score
    from team_scores
  )
  ...
  ranked_players as (
    select
      id,
      row_number() over (order by score desc, recent_matches desc, ign asc)::int as rank,
      score
    from player_scores
  )
  insert into tmp_new_rankings (entity_type, entity_id, rank, score)
