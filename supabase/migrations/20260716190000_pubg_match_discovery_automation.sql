...
create table if not exists public.pubg_player_watchlist (
  ...
  shard text not null default 'steam' check (shard in ('steam', 'kakao', 'xbox', 'psn')),
...
create table if not exists public.pubg_match_discovery_queue (
  ...
  shard text not null check (shard in ('steam', 'kakao', 'xbox', 'psn')),
  source_account_id text not null,
  status text not null default 'discovered'
    check (status in ('discovered', 'importing', 'imported', 'skipped', 'failed')),
...
revoke all on table public.pubg_player_watchlist from public, anon, authenticated;
revoke all on table public.pubg_match_discovery_queue from public, anon, authenticated;

grant select, insert, update, delete on table public.pubg_player_watchlist to service_role;
grant select, insert, update, delete on table public.pubg_match_discovery_queue to service_role;
